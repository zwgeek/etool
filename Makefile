...
DEBUGDONE = @make -f Config.mk SRCS_PATH=$(SRCS_PATH) INCLUDES=$(INCLUDES) TEST_SRCS_PATH=$(TEST_SRCS_PATH) debug;
...
debug :
	$(DEBUGDONE)
	@for platform in $(PLATFORM); do make -f Config.mk SRCS_PATH=$(SRCS_PATH) INCLUDES=$(INCLUDES) TEST_SRCS_PATH=$(TEST_SRCS_PATH) CROSS=$$platform- debug; done
